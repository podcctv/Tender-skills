...
目标：确认招标文件、附件、图纸、澄清文件、响应格式、模板和证据材料是否齐全，并先判定投标模式。

推荐提示词：

```text
使用 $tender-bid-writer 整理当前工作区的招标资料。请识别招标文件、附件、澄清文件、格式模板和证据材料，判断本项目属于 goods-mode、software-platform-mode、hybrid-goods-software-mode 还是 service-mode，说明依据，建立 00_source 到 06_delivery 的工作目录，并列出缺失或无法读取的文件。
```

阶段产物：资料清单、投标模式判断表、缺失文件清单、风险提示、工作目录。
